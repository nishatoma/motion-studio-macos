...
**Faster drafts:** Quick test renders 512×288 / 41 frames with 12 diffusion steps and guidance 1.0, which uses MLX-Video's single forward pass. It is the fastest compatibility check and can follow the prompt less faithfully or look rough. Fast detail keeps normal guidance at 768×448 / 49 frames but uses 16 steps instead of 40. The existing Preview uses 20 steps at 512×288; Detail and HD use 40. Fewer steps reduce sampling time but do not shorten model loading, image encoding, or VAE decoding. A job already in progress cannot switch presets. Generation time still depends on memory pressure and the Mac.
