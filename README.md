...
- **Render error:** use the always-visible **Show latest render log** button beneath the status, or download `motion-studio.log` from the same job folder under Movies → Nisha Motion Graphics. On failure, the log opens automatically with the full Manim output and Python traceback.
- **Old UI/error persists:** stop the running dashboard with Control-C in its Terminal window, relaunch `start.command`, then hard-refresh the browser with Command-Shift-R. Confirm the header says **BUILD 2026.09.25.5**. If the Terminal says port 8765 is already in use, an older dashboard is still running; close its Terminal window with Control-C first.
